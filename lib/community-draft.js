...
/**
 * Markdown mínimo → HTML de TipTap (el shape que guarda el producto en posts.content).
 * Deliberadamente pequeño: párrafos, negritas, links, y nada más. Un post de
 * comunidad no necesita tablas ni encabezados; si los necesita, se escriben a mano.
 */
export function mdToTiptapHtml(md) {
  ...
  return md
    ...
    .filter(Boolean)
    .map((block) => {
      // varias líneas seguidas (bullets 🔹 / ·) = un solo párrafo con <br>
...
/** HTML de TipTap → markdown mínimo. La inversa: se usó para migrar el borrador que vivía en la BD. */
