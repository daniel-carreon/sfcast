...
/** true solo si p está DENTRO de root (frontera de separador explícita — un hermano
 *  "root-evil" comparte prefijo de string con "root" y un startsWith desnudo lo dejaría pasar). */
...
function safeJoin(root, urlPath) {
  ...
  if (!insideRoot(root, resolved)) return null; // path traversal (incluye %2e%2e crudos)
  return resolved;
}
...
export async function serveFile(req, res, filePath) {
  ...
  // SIN Access-Control-Allow-Origin: la app es same-origin; un '*' dejaría a cualquier
  // web abierta en la misma máquina leer los media locales (hallazgo revisión 18 jul).
  const headers = {
    'Content-Type': mime,
