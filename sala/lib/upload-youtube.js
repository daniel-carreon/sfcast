...
async function fillTextbox(page, containerSel, text) {
  ...
  // Texto largo (descripción ~2000 chars): type char-por-char se cuelga/detacha (>30s) y aborta
  // la subida. Inserción instantánea vía execCommand (dispara los input events de YouTube).
  const ok = await box.evaluate((el, t) => {
    el.focus();
    try { return document.execCommand('insertText', false, t); } catch { return false; }
  }, text).catch(() => false);
  if (!ok) await box.type(text, { delay: 5, timeout: 120000 });  // fallback con timeout amplio
  await page.waitForTimeout(400);
}

// espera a que la subida del archivo termine (progreso deja de decir "Subiendo N%" → "Procesando"/
// "Comprobaciones"/"Se subió"). Poll con timeout amplio; el máster grande tarda en conexión lenta.
async function waitUploadFinished(page, shotsDir, timeoutMs = 45 * 60 * 1000) {
  const deadline = Date.now() + timeoutMs;
  let lastPct = -1;
  while (Date.now() < deadline) {
    const labels = await page.locator('.progress-label, ytcp-video-upload-progress, span.ytcp-video-upload-progress')
      .allInnerTexts().catch(() => []);
    const all = labels.join(' · ').toLowerCase();
    const m = all.match(/(\d+)\s*%/);
    if (m && +m[1] !== lastPct) { lastPct = +m[1]; process.stdout.write(`  subiendo… ${lastPct}%\n`); }
    const uploading = /subiendo|uploading/.test(all) || (m && +m[1] < 100);
    const finished = /procesando|comprobaciones|se subió|se ha subido|subida.*complet|processing|checks complete|uploaded/.test(all);
    if ((finished && !uploading) || (all && !uploading && !m)) {
      process.stdout.write('  ✓ archivo subido (100%) — el procesamiento sigue en YouTube.\n');
      await shot(page, shotsDir, '08b-subida-100.png');
      return true;
    }
    await page.waitForTimeout(4000);
  }
  process.stdout.write('  ⚠ timeout esperando 100% de subida (el draft puede quedar parcial).\n');
  return false;
}
...
async function fillUploadDialog(page, shotsDir, { title, description, defaults }) {
  ...
  // ⛔ ESPERAR a que el ARCHIVO suba 100% ANTES de cerrar (cerrar a media subida la aborta).
  await waitUploadFinished(page, shotsDir, 45 * 60 * 1000);

  // Un canal MONETIZADO añade un paso "Monetización" obligatorio que traba el flujo Next→Listo.
  // Vía robusta e independiente de la monetización: CERRAR el diálogo deja el video como
  // BORRADOR PRIVADO (el default de toda subida) — que es exactamente el "draft privado" buscado.
  // Daniel decide visibilidad/monetización al abrir el borrador para publicar.
  // cerrar el diálogo es MEJOR ESFUERZO: al llegar a 100% YouTube ya guardó el video como
  // BORRADOR PRIVADO ("Guardado como privado" en el header), aunque no cerremos el diálogo.
  const closeBtn = page.locator([
    'ytcp-uploads-dialog #close-icon-button', 'ytcp-uploads-dialog tp-yt-icon-button#close-button',
    'ytcp-uploads-dialog #close-button', 'ytcp-uploads-dialog-close-button',
    '#close-button', 'ytcp-button#close-button',
    'tp-yt-icon-button[aria-label="Cerrar"]', 'button[aria-label="Cerrar"]', 'ytcp-icon-button[aria-label*="errar"]',
  ].join(', ')).first();
  if (await closeBtn.isVisible({ timeout: 8000 }).catch(() => false)) {
    await closeBtn.click().catch(() => {});
    await page.waitForTimeout(1500);
    // confirmar "Guardar borrador" si YouTube lo pregunta al cerrar
    const saveDraft = page.locator(
      'ytcp-button:has-text("Guardar borrador"), ytcp-button:has-text("Guardar como borrador"), ' +
      'ytcp-button:has-text("Save draft"), #save-continue-button, tp-yt-paper-button:has-text("borrador")'
    ).first();
    if (await saveDraft.isVisible({ timeout: 4000 }).catch(() => false)) await saveDraft.click().catch(() => {});
  }
  await page.waitForTimeout(2000);
  await shot(page, shotsDir, '08-borrador-guardado.png');
}
...
export async function uploadFlow(projectDir, pub, { test = false, root, file = null } = {}) {
  ...
  try {
    ...
    // verificar que el BORRADOR quedó en Content (evidencia) — busca por el título
    process.stdout.write('verificando el borrador privado en la lista de contenido…\n');
    const titulo = pub.video?.titulo || md.titles?.[0] || '';
    const cid = channelIdFrom(page);
    await page.keyboard.press('Escape').catch(() => {});
    if (cid) await page.goto(`${STUDIO}/channel/${cid}/videos/upload`, { waitUntil: 'domcontentloaded' }).catch(() => {});
    await page.waitForTimeout(5000);
    await shot(page, shotsDir, '09-borrador-en-content.png');
    const rowSel = titulo ? `ytcp-video-row:has-text("${titulo.slice(0, 28)}")` : 'ytcp-video-row';
    const exists = await page.locator(rowSel).first().isVisible({ timeout: 8000 }).catch(() => false);
    setStage(pub, 'upload', 'done',
      `BORRADOR PRIVADO en el canal (Videos, 16:9)${exists ? ' — verificado en Content' : ''}: ${path.basename(videoFile)}. Daniel abre el borrador para monetización/visibilidad y publica.`);
    await savePublish(projectDir, pub);
    process.stdout.write(`✓ borrador privado ${exists ? 'VERIFICADO' : 'guardado'} en el canal. Screenshots en ${shotsDir}\n`);
    return { branch: 'A', shotsDir, verified: exists };
  } catch (e) {
