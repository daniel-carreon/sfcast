...
/**
 * LAS DOS FASES (contrato de Daniel, 26 jul): el lanzamiento no son 9 etapas internas, son DOS
 * momentos suyos. FASE 1 = lo que él valida en UN feedback (corte, título, miniatura, descripción,
 * post). FASE 2 = lo que corre solo cuando él dio el OK (programar, publicar, anunciar).
 * El stepper de 9 chips decía cosas que él no opera; esto dice qué falta y de quién es la pelota.
 */
export function fases(pub, extra = {}, now = new Date()) {
  const d = pub?.data || {};
  const md = d.metadata || {};
  const pd = d.post_draft || {};
  const L = d.launch;
  ...
  return [
    {
      ...
      items: [
        ...
        { id: 'miniatura', label: 'Miniatura', ok: !!d.thumbnail?.chosen, falta: 'sin portada elegida',
          detalle: d.thumbnail?.chosen || null },
        { id: 'descripcion', label: 'Descripción', ok: !!md.description, falta: 'sin escribir' },
        { id: 'post', label: 'Post aprobado', ok: !!pd.approved_at, falta: pd.body ? 'escrito, sin tu firma' : 'sin escribir' },
      ],
    },
    {
...
/**
 * Ruta absoluta de una miniatura por NOMBRE, mirando thumbs/ Y thumbnails/.
 * `listThumbs` devuelve UNA sola carpeta (la primera con archivos) porque ese es el set de
 * candidatas que se muestra; pero la portada elegida pudo quedar en la otra si el proyecto tiene
 * las dos. Devuelve null si no existe en ninguna.
 */
export async function resolveThumb(dir, name) {
  if (!name || name.includes('/') || name.includes('\\') || name.includes('..')) return null;
  for (const sub of ['thumbs', 'thumbnails']) {
    const fp = path.join(dir, sub, name);
    try { await fsp.access(fp); return fp; } catch { /* siguiente */ }
  }
  return null;
}
...
export async function readDossier(entry, now = new Date()) {
  ...
  return {
    ...
    // `html` = EXACTAMENTE lo que verá la comunidad: el mismo converter que usa el publicador.
    // Un preview con otra implementación mentiría, y aprobar a ciegas es justo lo que no queremos.
    post: { ...card.post, ...post, approved_at: pd.approved_at || null, title: pd.title || card.titulo,
      html: pd.body ? mdToTiptapHtml(pd.body) : '' },
    description,
...
export async function applyGalleryPatch(dir, patch, now = new Date()) {
  ...
  // la descripción se retoca a mano para detalles mínimos (una coma, un link): pasa a vivir en
  // publish.json, que es la fuente. Se guarda tal cual, sin reformatear nada.
  if (typeof patch.description === 'string') {
    const md = pub.data.metadata || {};
    if (md.description !== patch.description) {
      md.description = patch.description;
      md.description_edited_at = iso;
      pub.data.metadata = md;
      changed.push('descripción editada');
    }
  }
  if (changed.length) {
    pub.log = pub.log || [];
