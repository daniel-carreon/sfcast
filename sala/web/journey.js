...
const resultLabel=x=>({passed:'Comprobación superada',failed:'Requiere corrección',pending:'Pendiente'}[x]||x);
const freshnessLabel=x=>({matching:'Entradas coincidentes',invalidated:'Revisar de nuevo',unreadable:'Registro ilegible'}[x]||x);
...
function verificationDetails(check){
 ...
 return (check.verification||[]).map(v=>`<p><strong>${esc(methods[v.method]||v.method)} · ${esc(states[v.freshness]||v.freshness)}</strong><br>${esc(v.scope)}<br>Resultado registrado: ${esc(resultLabel(v.result))}</p>${v.reasons.length?`<p>${esc(v.reasons.join('; '))}</p>`:''}${v.evidence.map(e=>{
...
function resourceRow(r){return `<li>${r.state==='available'?`<a target="_blank" rel="noopener" href="/gallery/resource?id=${encodeURIComponent(item.id)}&resource=${encodeURIComponent(r.id)}">${esc(r.label)} ↗</a>`:esc(r.label)}<small>${r.state==='available'?'Archivo disponible':'Archivo ausente'}</small></li>`;}
function render(){
  ...
  $('detail').innerHTML=`<div><h2>${esc(stage.question)}</h2>${selected===2?creativePanel():''}<details ${selected===2?'':'open'}><summary>Explorar el método</summary>${nodes.map(n=>`<details><summary>${esc(n.label)}</summary><p>${esc(n.resumen||n.desc)}</p><details><summary>Detalle técnico</summary><small>${esc(n.desc||n.ruta||'Sin detalle adicional')}</small></details></details>`).join('')}</details></div>
    <div class="evidence">${selected===4?`<p>Publicación: ${esc(item?.launch?.label||'sin evidencia')}</p>`:''}<h3>En este video</h3><ul>${resources.filter(r=>r.kind!=='export').map(resourceRow).join('')||'<li>No hay recursos vinculados a esta etapa todavía.</li>'}</ul>
