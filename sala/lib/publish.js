...
/**
 * El slug de atribución = LOS ÚLTIMOS 6 DEL ID DE YOUTUBE. Decisión de Daniel, 27 jul 2026
 * ("dejalo como los ultimos 6 del id. punto. minusculas").
 * El link que se dice en el video queda `saasfactory.so/<slug>` — sin `/go/`, sin `vid-`, sin fecha.
 *
 * ⛔ El `_` NO es opcional convertirlo: la tabla `tracked_links` tiene un CHECK `slug_format` que
 * solo acepta `[a-z0-9-]`, y un id de YouTube sí puede traer `_` (este video: `0U4SfWOY2_k`).
 * Es la misma transformación que el generador del producto hace desde feb 2026.
 */
export function slugFromYoutubeId(ytid) {
  return String(ytid).slice(-6).toLowerCase().replace(/_/g, '-');
}

/** Reconoce el link de atribución en un texto: la forma nueva (raíz + 6) y la histórica (/go/). */
export const LINK_ATRIBUCION = /saasfactory\.so\/(go\/[a-z0-9-]+|[a-z0-9-]{6})(?![a-z0-9-])/i;

/** La URL pública de atribución. UN solo lugar que la arma. */
export function trackedUrl(slug) {
  return `https://saasfactory.so/${slug}`;
}
...
// ---------- EDL: mapeo tiempo RAW → tiempo del CORTE FINAL ----------
// El transcript word-level es del RAW (con retakes); la sala y YouTube viven en el corte final.
// edl_breathed.json: {ranges: [{start, end}]} en tiempo raw, EN ORDEN FINAL. Sin esto, los
// timestamps del dossier/capítulos/menciones mentirían (gotcha cazado en el v5: 66 min raw vs 40 final).
export function edlMapper(ranges) {
  let acc = 0;
  const segs = ranges.map((r) => {
    const s = { start: r.start, end: r.end, finalStart: acc };
    acc += r.end - r.start;
    return s;
  });
  return {
    finalDuration: acc,
    toFinal(t) {
      for (const s of segs) if (t >= s.start && t < s.end) return s.finalStart + (t - s.start);
      return null; // quedó fuera del corte
    },
  };
}

// filtra el transcript al material que SOBREVIVIÓ el corte, con tiempos del video final
export function applyEdl(words, ranges) {
  const map = edlMapper(ranges);
  const out = [];
  for (const w of words) {
    const t = map.toFinal(w.start);
    if (t !== null) out.push({ text: w.text, start: Math.round(t * 100) / 100, end: Math.round((t + (w.end - w.start)) * 100) / 100 });
  }
  out.sort((a, b) => a.start - b.start);
  return { words: out, duration: map.finalDuration };
}

// ---------- segmentación del transcript (para el dossier ⌘Y: párrafos con timestamp) ----------
// Corta por pausa larga (>gap s) o por longitud (maxWords) — legible sin perder el mapa temporal.
export function segmentTranscript(words, opts = {}) {
  const gap = opts.gap ?? 0.9;
  const maxWords = opts.maxWords ?? 26;
  const segs = [];
  let cur = null;
  for (const w of words) {
    const startNew = !cur || (w.start - cur.end > gap) || cur.n >= maxWords;
    if (startNew) {
      if (cur) segs.push({ t: Math.round(cur.t * 10) / 10, text: cur.text.join(' ') });
      cur = { t: w.start, end: w.end, n: 0, text: [] };
    }
...
export function checklistGate(pub, defaults = {}) {
  ...
  const checks = [
    ...
    // el link de atribución: raíz + 6 chars (saasfactory.so/woy2-k). Se acepta el /go/ histórico
    // para no romper proyectos viejos que ya lo traen escrito en su descripción.
    { id: 'descripcion_go', level: 'HARD',
      ok: LINK_ATRIBUCION.test(firstTwoLines),
      detail: LINK_ATRIBUCION.test(firstTwoLines)
        ? 'link de atribución en las primeras 2 líneas'
        : 'el link saasfactory.so/<slug> NO está en las primeras 2 líneas' },
