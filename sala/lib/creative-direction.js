...
const reviews={intro_in_motion:'Intro en movimiento',face_clear:'Rostro libre',mobile_legibility:'Lectura móvil',audio_sync:'Sincronía',reference_comparison:'Comparación con referente'};
export function projectDirection(doc){
 ...
 const checks=Object.entries(reviews).map(([id,label])=>{
  ...
  return {id,label,evidence,status:value?.checked===true&&evidence?'reported':'pending'};
 });
...
export async function creativeDirection(dir){
 ...
 try{
  ...
  return {state:'available',...projectDirection(JSON.parse(await fs.readFile(real,'utf8')))};
 }catch(e){
