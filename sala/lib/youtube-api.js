 ...
 */
/**
 * Escribe metadata (y opcionalmente PROGRAMA) el video.
 * `publishAt` null/undefined = **vestir sin programar**: título, descripción y tags quedan puestos y
 * el video sigue PRIVADO sin fecha. Es el estado en que termina la Fase 1: todo listo, esperando la
 * hora que solo da Daniel. Programar es una acción hacia afuera; no se hace sin que él la pida.
 */
export async function scheduleVideo(env, videoId, { publishAt, title, description, tags, categoryId }, token = null) {
  const t = token || await getAccessToken(env);
  const programar = publishAt !== null && publishAt !== undefined && publishAt !== '';
  let when = null;
  if (programar) {
    when = new Date(publishAt);
  ...
  const body = {
    ...
    status: {
      ...
      ...(programar ? { publishAt: when.toISOString() } : {}),
