 ...
 */
/**
 * ¿ya existe este post en la comunidad? Busca por título exacto + categoría + autor en las últimas
 * 24 h. Es el candado anti-doble-publicación entre máquinas.
 */
export async function findRecentPost(env, title, categoryId, authorId, horas = 24) {
  const desde = new Date(Date.now() - horas * 3600e3).toISOString();
  const q = `posts?select=id,title,created_at&category_id=eq.${categoryId}` +
    `&author_id=eq.${authorId}&created_at=gte.${desde}&title=eq.${encodeURIComponent(title)}&limit=1`;
  const r = await fetch(`${env.SF_SUPABASE_URL}/rest/v1/${q}`, {
    headers: { apikey: env.SF_SUPABASE_KEY, Authorization: `Bearer ${env.SF_SUPABASE_KEY}` },
  });
  if (!r.ok) throw new Error(`buscar post existente → ${r.status}: ${(await r.text()).slice(0, 200)}`);
...
export async function publishCommunityPost(env, {
  ...
  const payload = {
    ...
    // el video se EMBEBE y se reproduce dentro del post (el mismo campo donde va un Loom).
  ...
  // `publish.json` marca lo publicado, pero DOS máquinas tienen DOS copias: si el cron del Mac Mini
  // y un watcher de la MacBook corren a la vez, cada uno se ve "sin publicar" e insertan DOS posts a
  // ~1,700 personas. La única verdad compartida es la tabla `posts`: se pregunta ahí antes de escribir.
  const yaSalio = await findRecentPost(env, postTitle, categoryId, authorId);
  if (yaSalio) {
    return { id: yaSalio.id, title: postTitle, url: `${COMMUNITY_URL}?post=${yaSalio.id}`,
             chars: content.length, alreadyPublished: true, publishedAt: yaSalio.created_at };
  }
